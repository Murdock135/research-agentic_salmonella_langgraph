...
def extract_last_expression(code: str) -> Tuple[Optional[List[str]], str, Optional[SyntaxError]]:
    ...
    """
    logger.debug("Extracting last expression from code: \n%s", code)

    # Return early if the code is empty or only contains whitespace
    if not code or not code.strip():
        logger.debug("Empty code. Returning early")
        return None, "", None

    try:
        parsed_code = ast.parse(code)
    except SyntaxError as e:
        logger.debug("SyntaxError encountered while parsing code: %s", e)
        return None, "", e
    
    body = parsed_code.body
    if not body:
        logger.debug("Parsed AST body is empty")
        return None, "", None

    last_node = body[-1]
    logger.debug("Last node type: %s", type(last_node).__name__)
